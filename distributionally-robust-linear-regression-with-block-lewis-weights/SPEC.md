...
- T5 (sanity/unit checks): |f̃ − f| ≤ β log m + δ on random x (Lemma 6.1); E6 overestimate holds numerically and ‖w‖₁ ≤ 2(d+1); E7 sandwich holds numerically on random (x, c); ball-oracle outer iterates are non-increasing in F; E9 init has F(x₀) ≤ √(2(d+1))·OPT.
  - **Round-14 test-coverage resolution (T5 items the paper's maths imply but were not previously pinned):** an adversarial orchestration review confirmed three T5/E4/E5 invariants had no automated test despite the code being correct. All three are now pinned in `tests/test_invariants.py` (no implementation code changed):
    - **E9 Lewis warm-start quality** (SPEC T5 / `other_proofs.tex:51-61` Lemma `gp_regression_initialization`): `test_lewis_warm_start_init_quality` asserts `f(x₀) ≤ √(2(d+1))·f(x★)` on the unsquared `‖·‖_{G_∞}` scale (the lemma's scale), on a problem with `m=20 > 2(d+1)=10` so the E11 reset does **not** fire and the Lewis geometry is the one actually bounded. (Choice disclosed: the SPEC T5 wording "F(x₀) ≤ √(2(d+1))·OPT" mixes squared `F/OPT` with the `√` — the lemma is stated on the unsquared `f=‖Ax−b‖_{G_p}` scale; the test uses the unsquared scale, which is what the lemma actually proves, and is a valid consequence of it. The tighter `√(2 rank(A))` lemma bound is also asserted.)
    - **E5 p-objective Hessian** (`interpolation.tex:24-29`): `test_p_hessian_finite_diff_symmetry_psd` (p ∈ {2,4,8}) finite-differences the `[d,d]` Hessian from the gradient, asserts symmetry and PSD (`f=Σ‖r_i‖^p` convex).
    - **softmax weights `s ∈ Δ^m`** (E4 / `body.tex:285-288`): `test_softmax_weights_in_simplex` asserts `s ≥ 0` and `Σs = 1` incl. the max-shift edge case.
...
- **U19 (paper-internal inconsistency, synthetic eigenbasis)**: `experiments.tex:19` states "Each group, therefore, has a quadratic loss whose Hessian shares eigenvectors with the others but whose eigenvalues vary across groups" — i.e. every group Hessian is diagonalized by the shared orthonormal basis U. But `experiments.tex` *also* states phenomena that are only reproducible when the adversarial sharp directions are **misaligned/conflicting**, not orthogonal: phenomenon 2 ("The curvature directions that dominate different groups are misaligned") and phenomenon 3 ("the empirical risk minimizer … can perform well on most groups while incurring substantial loss on a small number of adversarial groups"), plus the measured "clear gap" of `experiments.tex:38`. If all Hessians share eigenvectors U and each adversarial sharp direction is a distinct U column, the sharp directions are orthogonal (not "misaligned"), ERM fits every adversarial group on its own independent axis (its worst group becomes a *normal* group, not an adversarial one), and the ERM-vs-robust gap collapses to ~6% — contradicting phenomenon 3 and the "clear gap". The three stated properties {shares eigenvectors; misaligned sharp directions; ERM incurs loss on adversarial} are mutually inconsistent under orthogonal eigenvectors. **Empirically verified** (Round-15, `solve_opt` via the E20 CVXPY epigraph QCQP, seed=0): the paper-literal shared-eigenbasis construction gives cond(AᵀA)=8.8e4 (✓ ~1e5) but F(ERM)/OPT=1.060 (gap 6%, ERM worst group = a *normal* group, idx 60 — phenomenon 3 fails); our rotated-spike construction (deviation below) gives cond=1.40e5 (✓) and F(ERM)/OPT=1.469 (gap 47%, ERM worst group = adversarial idx 4 — phenomenon 3 + the "clear gap" hold). **Decision**: reproduce the paper's *measured* phenomena (2, 3, the gap) rather than the literal "shares eigenvectors" phrasing, by drawing the adversarial sharp directions in a 2D subspace at spread angles (conflicting) so the rank-1 spike rotates the adversarial top eigenvector off U. Normal groups keep shared U exactly. This is a paper-internal inconsistency, not merely an unstated choice (cf. U11/U12/U14). Disclosed in `gdr/data_synthetic.py` docstring + `gdr/data.py` `deviation_note`; see §8A U1.
