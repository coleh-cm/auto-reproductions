"""Nesterov accelerated gradient on the LSE-smoothed surrogate (E3).

Paper: smoothed Nesterov baseline (E22, experiments.tex:60-66).  One
accelerated step per outer iteration (experiments.tex:99), standard
momentum form:

    y_k     = x_k + momentum * (x_k - x_{k-1})              # [d]  extrapolation
    x_{k+1} = y_k - lr * grad f_tilde(y_k)                  # [d]  gradient step
...
Note: the paper does not specify the Nesterov momentum schedule; we use a
constant grid momentum (disclosed choice, U2).
"""
...
def _run_single(problem, x0, max_outer, deadline, lr, momentum, beta, delta, opt):
    ...
    for k in range(1, max_outer + 1):
        ...
        if time.perf_counter() > deadline:
            break
        y = x + momentum * (x - x_prev)    # [d]  Nesterov extrapolation point
        _val, g, _H = smoothed_grad_hess(problem, y, beta, delta)  # E4; g [d]
        x_new = y - lr * g                  # [d]  gradient step from y
...
def run(problem, cfg, x0, max_outer, time_budget):
    """Best-of-grid smoothed Nesterov acceleration.  See module docstring."""
    cfg = cfg or {}
