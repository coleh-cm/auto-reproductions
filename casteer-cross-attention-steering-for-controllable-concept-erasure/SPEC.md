...
For any claim whose verdict depends on a value the paper never states, the survival RANGE (not merely a point) is the evidence a reader needs. The ranges below are **declared** in `claims.json` (each claim's `sensitivity.plausible` / `sensitivity.survives`, and where a claim depends on a *reading* the paper never settles, its `committed_readings`); a verdict holding across most of the plausible range rests on a weak reading of the paper, one holding only near a chosen point rests on a strong reading (Bennett, arXiv:2301.12987). **Empirical sweeping of the diffusion claims was not performed here because the arms are BLOCKED on CPU (§13)** — the declared widths are what a GPU run must confirm. Recording them so the gate can check the width, not just the point.

| claim | unstated value / reading | plausible range | survives range | width note |
|---|---|---|---|---|
| `nudity_beats_all_prior` | i2p_images_per_prompt | [1, 2] | [1, 2] | full width — the count doubles at k=2 but 14−18=−4<0 still holds (weak reading) |
| `nudity_two_times_fewer` | i2p_images_per_prompt | [1, 2] | **[1]** | **partial width — the count doubles at k=2 so 2×14−18=10>0 FAILS at k=2; survives only at k=1 (the adopted protocol, U7). The prior `survives=[1,2]` was arithmetically self-inconsistent (review F-5/#7a, fixed).** |
| `i2p_overall_beats_receler` | i2p_images_per_prompt | [1, 2] | [1, 2] | full width (mean-over-images metric; k∈{1,2} leaves the expectation unchanged) |
...
| `i2p_overall_beats_receler` | **eq9_renormalization (reading)** | {as_is, renormalize} | **{as_is}** | **essentially zero width — the adopted as-is reading (paper-literal "simply averaging", supplementary.tex:1068-1071) is the likely-REFUTE reading for this 1.42pp-margin claim, because the UPSTREAM code that produced the paper's I2P number (25.58) renormalized `b` at steer time (applying ~1/||mean||≈2.6× MORE erasure). This is the run's largest free extension of the text; it is recorded here (and in claims.json `committed_readings`) because a surviving range of zero width is what fitting-to-a-known-answer looks like from outside (Bennett). A GPU run settles the claim under as_is; under renormalize it would likely reproduce.** |
...
| `snoopy_erasure_ordering`, `snoopy_erasure_vs_doco`, `snoopy_preservation_vs_esd_receler`, `fig_clip_shape` | clip_cs_checkpoint_dim | [512, 768] | [512, 768] | full width — per-seed normalization by our own sd14 CS (experiments.tex:75) damps the checkpoint shift below the margins. The CS reference text is the BARE concept string (vendored produce_scores.py:43 -> `[concept]*N`), matching the paper's pipeline; our CS scale is w·cos (~0.5 range) vs the paper table's ×100 (~78 range) — the normalization cancels both (review F-6, fixed) |
| `snoopy_fid_preservation_vs_esd_receler`, `fig_fid_shape` | eval_images_per_concept | [200, 800] | [200, 800] | full width (margin ≥22.6 FID vs subset noise ≤3). The driver now generates the declared 800/concept (80 templates × 10, experiments.tex:67) via `n_per` expansion, not 80 (review: dead `n_per`, fixed) |
| `nudity_total_value` | i2p_prompt_subset_size | [1000, 4703] | [2000, 4703] | partial width — below 2000 the scaled-count SE exceeds the tolerance (inconclusive). `nudity_total` is now recorded on the FULL-SET basis (scaled; raw under `nudity_total_raw`) via `scale_count_to_full_set`; the driver reports BLOCKED (inconclusive) below 2000 instead of emitting a deflated number (review: nudity_total never scaled, fixed) |
| `style_vangogh_lpips_ordering` | lpips_eval_images | [200, 1000] | [200, 1000] | full width (margin 0.04 vs SE ~0.005 at 200). The driver now generates the declared 200 style images (50 classes × 4), not 50 (review, fixed). LPIPS inputs are resized to 256×256 — a REAL transform (LPIPS is resolution-dependent); both sets resized identically so pairing is preserved (review F-8, comment corrected) |
...
The one READING this reproduction adopted that diverges from the code which produced the paper's numbers is the Eq.9 multi-concept average (U9 / §14: used as-is, no renormalization). It is text-literal ("simply averaging", supplementary.tex:1068-1071) and conservative (weaker erasure), but the paper's I2P number (25.58) was produced by the renormalized upstream path. Per §12's own standard that fork must enter the declared sweeps — it does, in `i2p_overall_beats_receler.committed_readings` and the row above, with a surviving range of {as_is} (zero width). A reader needs to see that width: the verdict depends on a reading the paper's numbers were NOT produced under, and a GPU run under the as-is reading is the stringent test.
...
- **`measured.json` records `BLOCKED` for every diffusion-dependent metric** with a reason sidecar (`measured_blocked_reasons.json`). `run_all_arms.sh` detects the CPU host via `is_arm_feasible` and emits `FINAL <arm>=BLOCKED` per arm/seed without downloading any model. On a CUDA host the same script runs the arms for real via the implemented `_run_and_evaluate` driver: the per-task steering vector is selected via `TASK_VECTOR`, the 7-concept Eq.9 average for the I2P-overall task is composed on the fly from the 7 per-concept stores, I2P generation uses the dataset's per-prompt curated `sd_seed` (vendored run_i2p_eval.py:71), `nudity_total` is recorded on the full-set basis (scaled; raw under `nudity_total_raw`) and reports inconclusive below the 2000-prompt floor, snoopy/other/style generate the declared image counts (80×10/50×4) via `n_per`, the CLIP reference text is the bare concept string (produce_scores.py:43), and the sd14 normalization reference for snoopy/other/style is generated on demand per steered arm so `norm_snoopy_cs`/`mean_norm_others_cs`/`mean_others_fid`/`vangogh_lpips_e` are reachable on a GPU host (the prior driver structurally blocked them on any host). Two metrics remain BLOCKED on every host until their references are supplied: `i2p_overall_pct` (the Q16 classifier checkpoint is not named by the paper and not vendored; `q16_inappropriate_count` raises by construction) and `coco_fid30k` (the real COCO-30k FID reference is not vendored; set `COCO_REF_DIR` to the 30k real COCO images). Both are documented in `measured_blocked_reasons.json`. These are honest structural blocks, not "CPU-only" blocks.
...
## 15. Second review pass (this commit) — driver + eval-path corrections

The second adversarial review (three reviewers, converged) found that the equation-level implementation is faithful but the **GPU driver would silently mis-settle or structurally block the paper's headline claims**. The CPU-only BLOCKED verdicts were honest, but six of them were blocked for *structural* reasons mislabeled as "CPU-only", and several number-moving divergences from the paper's measurement protocol were undocumented. All are fixed here, covered by new pure-function tests + 8 new mutations (16/16 caught, 0 surviving; 65 tests pass):

- **nudity_total full-set scaling (fixed, review divergence #1 / faithful F-2 / metric (a)):** `core/eval/metrics.py::scale_count_to_full_set` + `nudity_scaled_or_inconclusive` put the subset count on the full-set basis (4,703) so it is commensurate with the paper's constants (18/7/646); the RAW count is recorded under `nudity_total_raw` (claims.json `metrics.nudity_total_raw`). Below the declared 2000-prompt floor (`NUDITY_INCONCLUSIVE_FLOOR`) the driver reports BLOCKED (inconclusive) instead of a ~4.7×-deflated number that could let the orderings pass for subset-bias reasons. The `FULL_CONFIG_MIN_IMAGES["i2p"]` is now 4,703 (the paper full set), so the full-set count is directly comparable. Tests: `test_scale_count_to_full_set`, `test_nudity_scaled_or_inconclusive_*`. Mutations: `scale_count_returns_raw`, `nudity_skip_inconclusive_floor`.
- **Structurally-unsettleable claims made reachable (fixed, review divergence #2 / faithful F-1 / metric §2/§5):** the sd14 normalization reference for snoopy/other/style is now generated **on demand** per steered arm at the same seed (lazily cached on disk) and scored in-memory; `norm_snoopy_cs` is lifted out of the `snoopy_cs` guard; `mean_norm_others_cs` and `mean_others_fid` use the in-memory sd14 per-concept references. The dead `_sd14_metric` (read stale `measured.json` from a previous run) and `_sd14_others_cs` (unconditional `return None`) are removed. `arm_metrics_from_claims` now parses `predicate` too, so the predicate-only `house` metrics are registered (review F-13). On a GPU host the 6 normalized-Snoopy claims are now reachable; on CPU they remain BLOCKED for the honest CPU reason (no longer a structural block). Tests: `test_arm_metrics_from_claims_parses_predicate`, `test_arm_metrics_from_claims_matches_real_claims_json`. Mutations: `arm_metrics_skip_predicate`.
- **Declared image counts via `n_per` (fixed, review divergence #3/#4/#5 / metric (b)):** `core/runner.py::expand_template_prompts` repeats each template `ceil(n_target/len(templates))` times so snoopy/other reach 800/concept (80×10, experiments.tex:67) and style reaches 200 (50×4), not 80/50 (the dead `n_per` previously left them below the >=200 restriction). Tests: `test_expand_template_prompts_*`. Mutation: `n_per_not_expanded`.
- **I2P per-prompt `sd_seed` protocol (fixed, review divergence #4 / faithful F-10 / metric (d)):** `core/data.py::load_i2p_seeds` exposes the curated per-prompt `sd_seed` column (vendored run_i2p_eval.py:71 — the protocol that produced the SD-1.4 Total=646 anchor); `run_generation` takes `prompt_seeds` and seeds a fresh generator per I2P prompt. The prior continuous-generator stream was an undocumented divergence that plausibly deflated nudity counts. Tests: `test_i2p_seeds_fingerprint`.
- **CLIP-score bare-concept reference text (fixed, review divergence #5 / faithful F-6 / metric (c)):** `core/eval/metrics.py::cs_reference_prompts` returns `[concept]*n`, matching the paper's pipeline (produce_scores.py:43 -> clip.py:191). The prior full-template scoring was not the quantity the paper's CS constants (78.5 etc.) were produced under. Tests: `test_cs_reference_prompts_is_bare_concept`. Mutation: `cs_reference_full_template`.
- **Eq.9 renormalization fork recorded (fixed, review divergence #6 / faithful F-3):** the as-is (no renorm) reading is text-literal and conservative, but the paper's I2P number was produced by the renormalized upstream path. The fork is now recorded in `claims.json::i2p_overall_beats_receler.committed_readings` and SPEC §12 with a surviving range of {as_is} (zero width) — a reader sees the verdict depends on a reading the paper's numbers were NOT produced under. The 7-concept string is corrected to `illegal activity` (supplementary.tex:1071; the prior `illegal` matched neither paper variant). Tests: `test_i2p_overall_is_the_seven_concept_average`. Mutation: `i2p_overall_concept_illegal`. The controller docstring is corrected: the dot-product (as-is) and matrix (`s s⁺`, renormalizing) paths AGREE only for unit `b` and DIVERGE for the Eq.9 sub-unit mean (review F-3).
- **Per-model steering-vector store (fixed, review divergence #7c / faithful F-7):** `build_controller` loads vectors from `<vector_dir>/<vector_model>` (sd14 arms -> `sd14`, sdxl_casteer_clip -> `sdxl-turbo`), so sdxl_casteer_clip cannot silently load SD-1.4's `nudity.pt`. Tests: `test_build_controller_per_model_vector_dir_isolates_sdxl_from_sd14`, `test_arm_config_vector_model_is_set_for_steered_arms`. Mutations: `vector_dir_not_per_model`.
- **Count-based sensitivity self-consistency (fixed, review divergence #7a / faithful F-5):** `nudity_two_times_fewer.sensitivity.survives` is [1] (at k=2 the count doubles, 2×14−18=10>0 fails), not the self-inconsistent [1,2]. `nudity_beats_all_prior` keeps [1,2] (14−18=−4<0 holds). The LPIPS comment is corrected: the 256×256 resize is a REAL transform (LPIPS is resolution-dependent), not a no-op; recorded in the style claim's sensitivity (review F-8). Duplicate `direction` keys in two claims removed (review F-13).
