...
**In progress — environment complete.**
...
- [x] Implementation / environment setup — `requirements.txt` (Python 3.13, every dep pinned), `Dockerfile` (CUDA 12.4 base), `README.md` (quickstart verified), `pyproject.toml` (pytest pythonpath), `.gitignore`. Import gate `tests/test_environment.py` (6 passed on a clean `uv venv --python 3.13 .venv && uv pip install --python .venv -r requirements.txt` rebuild): every third-party dep + every `core`/`scripts` module imports; `construct_prompts` builds 210 human-related pairs + 50 concrete pairs; SPEC `house` invariant (Householder norm-preservation under β=2, unit s) and `CrossAttentionOutputSteering` Eq. 5/6 closed-form match pass.
