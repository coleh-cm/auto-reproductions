...
- `measured.json` — the measured results, `{arm: {seed: {metric: value}}}`,
  covering every arm and seed in `claims.json`. Generated by `run_all_arms.sh`.
- `instruments.json` — every thing that decides whether an output is correct
  ...
  each with a positive and negative test.
...
- `tests/` — the degeneracy gate (`test_degeneracy.py`: the λ=0 path is bitwise
  ...
  equation-invariant tests (`test_invariants.py`), data and CLI tests,
  instrument tests (`test_instruments.py`), and mutation tests
  (`test_mutations.py` that injects each defect and asserts the suite catches
  it).
...
```bash
.venv/bin/python -m pytest -q     # 41 tests: degeneracy + invariants + data + CLI + instruments + mutations
```
