...
- `measured.json` — the measured results, `{_meta, <arm>: {<seed>: {<metric>:
  value}}}` (the `_meta` key is reserved and ignored by the gate), covering
  every arm and seed in `claims.json`. Generated by `run_all_arms.sh`; the
  metrics are accuracy plus the structural invariants (gate bounds, simplex,
  stop-grad, single-network, degeneracy) the numbers gate adjudicates.
...
- `SPEC.md` — algorithm spec, shapes, equation citations, unstated-items
  list, and a Constructed-truth section (§10).
