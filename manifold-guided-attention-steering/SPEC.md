...
8. **Held-out split for head selection**: the paper Section 3.4 (tex:L294–298) fixes a problem-level 70/30 train/test split — manifold built on the 70% train, evaluated on the 30% held-out — and Section 4 (tex:L304–305) selects top-K heads by held-out AUROC "on a held-out problem split." The most faithful reading is that head selection uses the **same 30% held-out** the manifold is evaluated on (the paper computes the Figure-3 diagnostic on that held-out test split too). `→ 70/30 problem-level split: 70% manifold-train / 30% held-out (used for BOTH head-selection mean-AUROC and the Figure-3 max-AUROC diagnostic)`. The earlier choice was a 70/15/15 carve-out (select + a separate report-only split to avoid selection-bias on the diagnostic); round-33 corrected this to the paper's 70/30 after an adversarial review confirmed the 15% select split was a number-affecting deviation (noisier head-AUROC estimates can flip the K=3 top-K ranking).
...
15. **ITI adaptation mechanics** beyond "correct/incorrect traces as contrast sets" (tex:L394) — probe type, intervened activations: unstated. `→ per-head logistic-probe on the trace-mean head output, top-K heads by held-out probe accuracy (K ∈ {24,48,96}); intervention `a += α·σ_h·v_h` every decode step (original ITI convention, Li et al. 2023); α ∈ {0.5,1,5}`.
   - **Round-33 correction (recorded honestly — a deviation, not a silent bug):** the paper says the ONLY adaptation to ITI is replacing the prompt-pair construction with correct/incorrect traces (tex:L394) and otherwise "follow[s] the hyperparameter ranges reported in the original paper" (tex:L605), i.e. the published ITI which fits a probe per head across **all `L×H` heads** (1024 for Llama-32l) and selects top-K. The reproduction instead ranks and selects ITI's top-K from the **monitored-layer heads only** (Llama `{8,16,24,31}×32` = 128 heads; `mags/baselines.py:103-156`, `mags/capture.py:88`, `mags/store.py:79`), because the shared capture pipeline records only the MAGS-monitored layers. This narrows the candidate pool (most acute at K=96: 96/128 vs 96/1024) and changes which heads land in the top-K, so the reproduced ITI ablation accuracies (Table 4b, tex:L640-642) are **not guaranteed to match** the paper's described procedure. Why not fixed in code: searching all layers needs an ~8×-larger all-layer capture (memory/time) that cannot be validated in this no-GPU environment and would risk the green test suite; the monitored-layer reuse is the same shared-capture adaptation already recorded for Angular Steering (§4.16). The faithful all-`L×H`-head search is the reference design if an all-layer capture path is later added. This is number-affecting for a GPU run and is flagged as a known faithfulness gap, not a plumbing bug.
...
17. **Contrastive Decoding specifics** — amateur model per family and CD hyperparameters: unstated. `→ α_plausibility=0.1, β=1.0`; amateur: Llama-3.2-1B-Instruct for Llama-3.1-8B-Instruct, gemma-3-1b-it for Gemma-4-E4B-it; excluded for GPT-OSS (paper's own exclusion, tex:L527)`. **β=1.0 (round-28 correction, verified):** the CD paper (Li et al. 2023, li2023contrastive, the paper MAGS cites at tex:L396) defines the contrastive objective as "the **difference between the likelihood** under [expert] and [amateur]" (abstract) — i.e. `log p_exp − log p_ama`, coefficient 1 on BOTH log-probs; there is **no β** coefficient on `log p_ama`. The amateur temperature τ (a SEPARATE op, `softmax(logits_ama/τ)`; =1.0 for OPT/Llama-class) is not a coefficient on `log p`. The prior "β=0.5 (CD paper defaults)" was wrong (the CD paper states no such coefficient); corrected to β=1.0 = the plain log-ratio. τ=1.0 is a no-op, so no amateur-temperature machinery is needed. Implemented in `mags/config.py::CD_DEFAULT_BETA=1.0`, `mags/baselines.py::ContrastiveDecoder`, `mags/generation.py::cd_generate`.
 18. **Molecular task specifics** — target protein, molecule prompts, SMILES corpus for contrastive pairs, high/low binding-affinity cutoff, AutoDock-GPU parameters, GPT-OSS precision: **all unstated**. `→ will be pinned at implementation time from AutoDock-GPU defaults; treat Table 3 as a stretch target.` **Validity metric implemented (round-28):** "fraction of generated SMILES parseable" (tex:L527) is well-defined and needs no unstated protein — `mags/grading.py::grade_smiles_validity`/`smiles_is_valid` parse via RDKit (pinned in requirements.txt), wired into the `grade()` dispatch. **Binding Affinity (AutoDock-GPU kcal/mol)** still needs the unstated target protein + AutoDock-GPU binary + docking params, so it remains a documented stretch block (`mags/run.py:250-255` blocks the whole molecular task upstream — no molecules generated — so neither metric produces numbers without a GPU host + the unstated setup).
...
- `contrastive-decoding`: `α_p=0.1, β=1.0` (the plain log-ratio; round-28 correction verified against the CD paper, SPEC §4.17), amateur per §4.17.
